...
def pegarDados(endereco):
    ...
    return {"headers":headers, "body":np.array(matriz), "ri":ri, "types":typ}
...
def imprimirEntrada(dados):
    body = np.hstack((dados["ri"], dados["body"], dados["types"]))
    matriz = np.vstack((dados["headers"], body))
